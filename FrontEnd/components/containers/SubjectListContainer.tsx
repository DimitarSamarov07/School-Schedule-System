...
export default function SubjectListContainer({ manager }: { manager: any }) {
    const {
        subjectList,
        isLoading,
        activeModal,
    ...
    return (
        <div className="p-10 bg-white rounded-2xl font-sans overflow-hidden ">
            <div className="flex justify-between items-end mb-8">
                <div>
                    <h1 className="text-4xl font-extrabold text-[#1A1A1A] tracking-tight mb-2">Предмети</h1>
                    <p className="text-gray-500 font-medium">Управлявайте вашите предмети</p>
                </div>

                <button
                    onClick={() => setActiveModal("add")}
                    className="bg-[#7C5CFC] text-white px-8 py-4 rounded-2xl font-bold hover:bg-[#6b4de0] transition-all shadow-lg shadow-purple-200 active:scale-95 flex items-center gap-3 cursor-pointer"
                >
                    <Plus size={20}></Plus>
                    Добави предмет
                </button>
            ...
            <div className="mb-8 max-w-2xl ">
                <div className="relative group">
                    ...
                    <input
                        type="text"
                        placeholder="Търсене по име на предмет..."
            ...
            <div className="bg-white rounded-[2.5rem] shadow-xl shadow-gray-100/50 border border-gray-100 overflow-hidden">
                <table className="w-full text-left">
                    ...
                    <tr className="text-gray-500">
                        <th className="px-10 py-6 font-black uppercase tracking-widest text-xs">Предмет</th>
                        <th className="px-10 py-6 font-black uppercase tracking-widest text-xs">Забележки</th>
                    ...
                    {subjectList.map((subject: Subject) => (
                        <tr key={subject.id} className="hover:bg-gray-50/50 transition-colors group">
                            <td className="px-10 py-7 text-[#1A1A1A] font-bold text-xl">{subject.Name}</td>
                            <td className="px-10 py-7 text-gray-600 font-medium text-lg">{subject.Description || ""}</td>

                            <td className="px-10 py-7">
                                <div className="flex justify-end gap-3">
                                    <button
                                        onClick={() => openEditModal(subject)}
                                        className="p-3 text-[#7C5CFC] hover:bg-purple-50 rounded-2xl transition-all cursor-pointer"
                                    >
                                        <Pencil className="w-6 h-6" />
                                    </button>
                                    <button
                                        onClick={() => openDeleteModal(subject)}
                                        className="p-3 text-[#E74C3C] hover:bg-red-50 rounded-2xl transition-all cursor-pointer"
                                    >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#1A1A1A]/40 backdrop-blur-sm">
                    <div className="bg-white rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl">
                        <h3 className="text-2xl font-black text-gray-900 mb-8">
                            {activeModal === "add" ? "Добави нов предмет" : activeModal === "edit" ? `Редакция: ${selectedSubject?.Name}` : "Изтриване"}
                        </h3>

                        {activeModal === "delete" ? (
                            <div className="space-y-8">
                                <p className="text-gray-600 text-lg leading-relaxed">Сигурни ли сте че искате да изтриете предмет <span className="font-bold text-black border-b-2 border-red-200">{selectedSubject?.Name}</span>?</p>
                            ...
                            <div className="space-y-6">
                                <div>
                                    <label className="block text-xs font-black text-gray-400 mb-2 uppercase tracking-widest">Име на Предмет</label>
                                    <input
                                ...
                                <div>
                                    <label className="block text-xs font-black text-gray-400 mb-2 uppercase tracking-widest">Забележки</label>
                                    <input
                                        className="w-full border-2 border-gray-100 bg-[#F9FBFF] rounded-2xl px-5 py-4 outline-none focus:border-[#7C5CFC] focus:bg-white transition-all text-lg font-semibold"
                                        value={formData.Description}
                                        onChange={(e) => setFormData({ ...formData, Description: e.target.value })}
                                        placeholder="напр. 1ви срок"
                                    />
                                </div>
                                <div className="pt-6 flex flex-col gap-4">
                                    ...
                                    <button onClick={closeModal} className="w-full text-gray-500 font-bold py-2 hover:text-gray-600 transition-colors cursor-pointer">Отказ</button>
                                </div>
                            </div>
                        )}
                    </div>
