...
export const UserCard: React.FC<UserCardProps> = ({ user, onPromote, onDemote, onRemove }) => {
    ...
    async function handleRemove() {
        ...
        try {
            await removeUserFromSchool(currentSchool.SchoolId, user.id);
            onRemove?.(user.id);
        } catch (err) {
        ...
        <div className="group bg-white/70 backdrop-blur-sm rounded-3xl p-6 border border-slate-200/50 shadow-xl hover:shadow-2xl transition-all hover:-translate-y-2">
            <div className="flex items-start gap-4 mb-4">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0 ${user.isAdmin ? 'bg-gradient-to-br from-purple-500 to-purple-600' : 'bg-gradient-to-br from-indigo-500 to-indigo-600'}`}>
                    <span className="text-2xl font-bold text-white">
                        {user.name.split(/[\s,_]/).map((n: string) => n[0]).join('').toUpperCase()}
                    </span>
                </div>
                <div className="flex-1 min-w-0">
                    ...
                    <div className={`inline-flex items-center gap-2 text-xs px-2 py-1 rounded-full font-medium ${
                        user.isAdmin ? 'bg-purple-100 text-purple-700' : 'bg-emerald-100 text-emerald-700'
                    }`}>
            ...
            <div className="flex gap-2 pt-4 border-t border-slate-200/50">
                ...
                <button
                    ...
                    className="flex-1 bg-slate-100 hover:bg-red-50 hover:text-red-600 hover:border-red-200 text-slate-700 py-3 px-4 rounded-xl font-medium transition-all border border-transparent"
                >
                    Премахни
                </button>
            </div>
