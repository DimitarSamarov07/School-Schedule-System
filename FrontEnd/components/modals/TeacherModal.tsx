...
export function TeacherModals({ manager }: { manager: any }) {
    if (!manager.activeModal) return null;

    if (manager.activeModal === 'delete') {
        return (
            <ModalShell title="Изтриване на учител" onClose={manager.closeModal} maxWidth="sm"
                        footer={<>
                ...
                <p>Сигурни ли сте, че искате да изтриете <strong>{manager.selectedTeacher?.Name}</strong>?</p>
            </ModalShell>
        );
    }
    ...
    return (
        <ModalShell
            title={manager.activeModal === 'add' ? "Добави Учител" : "Редактирай Учител"}
